...
export function getUserDisplayName(user?: IUser | null): string {
    ...
    return user.profile?.pseudonym || user.authorName || fullName || getUserHandle(user) || user.email;
}

export function getUserAvatar(user?: IUser | null): string | undefined {
    return toMediaUrl(user?.profile?.avatar || user?.currentAvatar?.image);
}
...
export function getUserBirthDate(user?: IUser | null): string | Date | null | undefined {
...
export function getPostImages(post: IPost): Array<{ id: number; url: string }> {
    return (post.images || [])
        .map((image) => ({
            id: image.id,
            url: toMediaUrl(image.compressed_image || image.original_image || image.url) || "",
        }))
...
export function getPostTags(post: IPost): string[] {
    ...
    return (post.hashtags || []).map((item) => item.hashtag?.title).filter(Boolean);
}
