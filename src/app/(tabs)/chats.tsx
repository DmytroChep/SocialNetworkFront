...
import { useGroupCreation, ModalUser } from "../../shared/context/group-creation-context";
    ...
    const groupChatItems = useMemo(() => {
        return (chatsResponse?.chats ?? [])
            ...
            .map((chat) => ({
                ...
                lastMessage:
                    chat.lastMessage?.text || (chat.lastMessage?.images?.length ? "Фото" : "Немає повідомлень"),
                time: formatChatTime(chat.lastMessage?.created_at),
                unreadCount: chat.unreadCount ?? 0,
            }));
    }, [chatsResponse?.chats]);
                    ...
                    {!activeChat && choosedTab === "Групові чати" && (
                        <View style={{ flex: 1 }}>
                            <View style={styles.sectionHeader}>
                                ...
                                <TouchableOpacity
                                    style={styles.createGroupTrigger}
                                    onPress={() => openStep1(friendsList)}
                                >
                                    <Text style={styles.createGroupTriggerText}>+ Створити</Text>
                                </TouchableOpacity>
                            </View>
                            <GroupChatsList
...
const styles = StyleSheet.create({
    container: {
        ...
        right: 0,
        bottom: 0,
        backgroundColor: 'white', // или rgba(0,0,0,0.5)
        },
    flexElement: {
        flex: 1,
